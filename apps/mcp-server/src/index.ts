...
/** Read-mostly MCP server whose only writes update existing fix-attempt metadata. */
...
server.tool(
  ...
  async () => {
    try {
      ...
      const text = projects
        .map((p) => `• ${p.name} - ${p.url || "(no URL)"} ${p.framework ? `[${p.framework}]` : ""}`)
        .join("\n");
      return {
  ...
  async ({ url }) => {
    try {
      ...
      const lines = [
        ...
        scan.accessibility_score != null ? `| Accessibility | ${scan.accessibility_score} |` : null,
  ...
  async ({ url }) => {
    try {
      ...
      const text = dismissed
        .map((d) => `• ${d.title ?? d.check_id} [${d.status}] (since ${d.last_status_changed_at})`)
        .join("\n");
      return {
        content: [
          {

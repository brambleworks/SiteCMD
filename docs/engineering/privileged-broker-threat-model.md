...
| `run_filesystem_access_command`  | filesystem access   | `update_project_path`, `open_path_in_editor`, `reveal_path`, `register_agent_tool`, `unregister_agent_tool`                                                                                                                                                                                                                                                                                                                                                                                                                                          | Same shared sensitive-issuer path as above.                                                                                                                                                              |
...
`launch_agent_handoff` is in the filesystem-access allowlist but not in its
sensitive list. It opens the coding agent's own app through a deep link with
the kickoff prompt staged in the composer; the agent never runs that prompt
on its own, so a system dialog would only stand between the person and the
button they just pressed. `launching_a_coding_agent_handoff_needs_no_native_intent`
in the broker tests keeps it that way.
